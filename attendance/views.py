...
from .services.recognition import RecognitionError, RecognitionResult, recognize_data_url
...
def dashboard(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "attendance/dashboard.html", {"courses": courses, "sessions": sessions})
...
def start_session(request: HttpRequest, course_id: int) -> HttpResponse:
    ...
    with transaction.atomic():
        ...
        records = [
            AttendanceRecord(session=session, student=enrollment.student, status=AttendanceRecord.Status.ABSENT, source=AttendanceRecord.Source.SYSTEM)
...
def live_session(request: HttpRequest, session_id: int) -> HttpResponse:
    ...
    return render(request, "attendance/live_session.html", {"session": session, "records": records, "confirmation_frames": settings.FACE_CONFIRMATION_FRAMES})


def _next_confirmation(session_id: int, student_id: str) -> int:
    """Track consecutive matched frames in Django's local cache for this dev prototype."""
...
def recognize_frame(request: HttpRequest, session_id: int) -> JsonResponse:
    ...
    if record.status == AttendanceRecord.Status.PRESENT:
        _reset_confirmation(session.id)
        return JsonResponse({
            "ok": True, "recognized": True, "student_id": result.student_id, "name": record.student.name,
            "distance": result.distance, "confirmed": True, "attendance_status": "already_present",
    ...
    if count < settings.FACE_CONFIRMATION_FRAMES:
        return JsonResponse({
            "ok": True, "recognized": True, "student_id": result.student_id, "name": record.student.name,
            "distance": result.distance, "confirmed": False, "count": count,
    ...
    return JsonResponse({
        "ok": True, "recognized": True, "student_id": result.student_id, "name": record.student.name,
        "distance": result.distance, "confirmed": True, "attendance_status": "marked_present",
...
def export_session_csv(request: HttpRequest, session_id: int) -> HttpResponse:
    ...
    for record in session.records.select_related("student").order_by("student__student_id"):
        writer.writerow([
            session.course.code, session.course.name, session.session_date,
            record.student.student_id, record.student.name, record.status, record.source,
            record.marked_at.isoformat() if record.marked_at else "", record.recognition_distance or "",
        ])
